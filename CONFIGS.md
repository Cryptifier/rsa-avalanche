...
- `config/rsa_config_small_public_key_demo_<modulus-bits>.json`: Generated by `scripts/run_small_public_key_demo.sh` from the small-batch config and wired to the first extracted PKESK ciphertext under `config/pgp/<modulus-bits>/`.
...
# Notes for generated public-key demo configs
- `scripts/run_small_public_key_demo.sh` writes a modulus-specific generated config beside the tracked configs, for example `config/rsa_config_small_public_key_demo_512.json`.
- The generated config starts from `config/rsa_config_small_batch.json`, switches `rsa_keypair.keyfile` to the generated public YAML, keeps `rsa_keypair.private_keyfile` as a verification peek, and enables `engine.message.use_file` plus `engine.message.is_encrypted`.
- `engine.message.fixed_file` points at the extracted PKESK ciphertext hex file written beside `config/pgp/<modulus-bits>/prep_blob_1.asc`.
- `engine.message.bits` is derived from the decrypted PKCS#1 v1.5 payload width, and `engine.avalanche_fitness_shift_bytes` is forced to `0` so the final global-majority hex can be compared directly against the stored payload hex artifact.
